		...
		switch(street) {
			...
			case 3:
				if (raised) {
					...
					double probBet = betFlop/(double)flopObs;
					double estimated = 1-probBet/2;
					return estimated;
				} else if (called) {
					double probCall = (1-foldFlop)/((double)flopObs);
			...
			case 4:
				if (raised) {
					double probRaise = raiseTurn/(double)turnObs;
					double estimated = 1-2*probRaise;
					return estimated;
				} else if (bet) {
					double probBet = betTurn/(double)turnObs;
					double estimated = 1-probBet/2;
					return estimated;
				} else if (called) {
					double probCall = (1-foldTurn)/((double)turnObs);
					double estimated = 1-probCall/2;
					return estimated;
				} else {
					double estimated = 0.5;
			...
			case 5:
				if (raised) {
					double probRaise = raiseRiver/(double)riverObs;
					double estimated = 1-2*probRaise;
					return estimated;
				} else if (bet) {
					double probBet = betRiver/(double)riverObs;
					double estimated = 1-probBet/2;
					return estimated;
				} else if (called) {
					double probCall = (1-foldRiver)/((double)riverObs);
					double estimated = 1-probCall/2;
					return estimated;
				} else {
					double estimated = 0.5;
